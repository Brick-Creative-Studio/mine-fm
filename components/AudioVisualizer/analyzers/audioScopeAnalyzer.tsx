...
import { useMicrophoneLink } from './utils/common'
import ScopeAnalyzer from './utils/scope'
import AudioScopeAnalyzerControls from './scopeAnalyzerControls'

interface InternalAudioScopeAnalyzerProps {
  audioSource: AudioSource
}
const InternalAudioScopeAnalyzer = ({ audioSource }: InternalAudioScopeAnalyzerProps) => {
  if (audioSource === AUDIO_SOURCE.MICROPHONE) {
    throw new Error('Use InternalMicrophoneScopeAnalyzer for microphone inputs.')
  }
...
const InternalMicrophoneScopeAnalyzer = ({}: InternalMicrophoneScopeAnalyzerProps) => {
  const audioCtx = useMemo(() => buildAudioContext(), [])
  const audio = useMemo(() => buildAudio(), [])
  const analyzer = useMemo(() => {
    return new ScopeAnalyzer(audio, audioCtx)
  ...
  const { onMicDisabled, onStreamCreated } = useMicrophoneLink(audio, analyzer)
...
const AudioFFTAnalyzer = ({}: AudioScopeAnalyzerProps) => {
  ...
  return (audioSource as unknown as AudioSource) === AUDIO_SOURCE.MICROPHONE ? (
    ...
    <InternalAudioScopeAnalyzer audioSource={audioSource as unknown as AudioSource} />
  )
}
