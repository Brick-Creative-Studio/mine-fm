...
const InternalAudioFFTAnalyzer = ({ audioSource }: InternalAudioAnalyzerProps) => {
  ...
  useEffect(() => {
    analyzer.volume =
      (audioSource as unknown as AudioSource) === AUDIO_SOURCE.MICROPHONE ? 0.0 : 1.0
...
const InternalMicrophoneFFTAnalyzer = ({}: InternalMicrophoneFFTAnalyzerProps) => {
  const audioCtx = useMemo(() => buildAudioContext(), [])
  const audio = useMemo(() => buildAudio(), [])
  const analyzer = useMemo(() => {
...
const AudioFFTAnalyzer = ({}: AudioFFTAnalyzerProps) => {
  ...
  return (audioSource as unknown as AudioSource) === AUDIO_SOURCE.MICROPHONE ? (
    ...
    <InternalAudioFFTAnalyzer audioSource={audioSource as unknown as AudioSource} />
  )
}
