...
import { BufferGeometry, CatmullRomCurve3, Points, Vector2, Vector3 } from 'three'
import {
  COORDINATE_TYPE,
  gaussianRandom,
  ICoordinateMapper,
...
const computeNormals = (vertices: Vector2[], weighted: boolean = false): Vector2[] => {
...
const BaseTraceParticles = ({
  ...
  const referenceNormalsByPoly = referencePositionsByPoly.map((referencePositions) =>
  ...
  useFrame(({ clock }) => {
    ...
    const nextPolyIdx = currPolyIdx + 1 >= polyStates.length ? 0 : currPolyIdx + 1
    const transitionAlpha = (elapsedTimeSec % transitionSpeedSec) / transitionSpeedSec
