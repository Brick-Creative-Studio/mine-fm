...
const BaseDoubleHelix = ({
  ...
  const geoBase = useMemo(() => {
    ...
    for (let i = 0; i < geo.attributes.position.count; i++) {
      geo.attributes.position.setZ(i, geo.attributes.position.getZ(i) - baseLength / 2)
    }
  ...
  const [curveHelixA, geoHelixA, curveHelixB, geoHelixB] = useMemo(() => {
    const curveA = new HelixCurve(helixLength, helixRadius, helixWindingSeparation, 0)
  ...
  }, [helixLength, helixRadius, helixWindingSeparation, strandRadius, strandOffsetRad])
  ...
  useFrame(({ clock }) => {
    ...
    for (let bpIdx = 0; bpIdx < nBasePairs; bpIdx++) {
      ...
      targetScaleA =
        targetScaleMin +
        (fixedBaseGap ? 2 * targetScale : targetScale) * (targetScaleMax - targetScaleMin)
      targetScaleB = fixedBaseGap
        ? targetScaleMin + 2 * (1 - targetScale) * (targetScaleMax - targetScaleMin)
        : targetScaleA
